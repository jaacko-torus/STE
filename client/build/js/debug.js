...
const DEBUG = {
  ...
  module_text(s, font, module) {
    // option to force if necesary
  ...
  show_angle_indicators(s, module, w_height, w_length) {
    if (this.var.show_angle_indicators) {
      ...
      s.line(module.position.x, module.position.y, // TODO: this only applies to d = 0 || 1
      module.position.x + Math.cos(module.angle) * (w_height - w_length), module.position.y + Math.sin(module.angle) * (w_height - w_length));
    }
  },
  ...
  show_d_variable(s, font, module) {
    ...
    s.text(module.position.d, module.position.x, module.position.y);
  },
  ...
  show_id(s, font, module) {
    ...
    s.textSize(9);
    s.textStyle(s.BOLD);
    s.textAlign(s.CENTER, s.CENTER); // FIXME: not sure what I was attempting below
