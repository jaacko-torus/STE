...
class Module {
  ...
  constructor(world, owner, spaceship, id, {
    x,
    y,
    d
  }, {
    ...
    _defineProperty(this, "owner", "");

    _defineProperty(this, "spaceship", "");
    ...
    _defineProperty(this, "neighbors", []);
    ...
    _defineProperty(this, "color", ["#333333", 0.5]);
    ...
    _defineProperty(this, "__id__", void 0);

    _defineProperty(this, "Matter", {
      meta: {
        owner: "",
        spaceship: undefined,
        __id__: ""
    ...
    this.level = level;
    this.interval = interval; // this.position = { x, y };
    ...
    this.angle = angle;
    this.size = size;
    this.__id__ = id ?? x + "," + y; // add to user's spaceship unless these are not specified
  ...
  static get w_height() {
    return Module.w_size * Math.sqrt(3) / 2;
  }

  static get w_length() {
    return Module.w_size * Math.sqrt(3) / 3;
  }

  static get h_size() {
    return Module.w_size / 2;
  } // half


  static get h_height() {
    return Module.h_size * Math.sqrt(3) / 2;
  }

  static get h_length() {
    return Module.h_size * Math.sqrt(3) / 3;
  }

  static get w_constraints() {
    // TODO: maybe replace these numbers with `w_size`, `w_height`, and `w_length`, might help
    let s = Module.w_size;
  ...
  static create(world, owner, spaceship, module, size) {
    ...
    if (size === 0.5) {
      length = Module.h_length;
      mass = 0.25;
    }

    if (size === 1) {
      length = Module.w_length;
      mass = 1;
    }

    let module_body = Matter.Bodies.polygon(module.position.x, module.position.y, 3, length, {
      angle: module.angle,
      mass
