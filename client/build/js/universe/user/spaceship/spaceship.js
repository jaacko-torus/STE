...
class Spaceship {
  constructor(world, owner, id, position, modules = [], angle = 3 * Math.PI / 2) {
    ...
    _defineProperty(this, "main", undefined);
    ...
    _defineProperty(this, "input_keys", {});

    _defineProperty(this, "capsules", new Map());

    _defineProperty(this, "modules", new Map());

    _defineProperty(this, "constraints", new Map());

    _defineProperty(this, "composite", void 0);

    _defineProperty(this, "projected_torques", void 0);
    ...
    this.owner = owner;
    this.position = position; // this.grid = { d: grid.d }
    // this.angle = angle;

    this.composite = Matter.Composite.create(); // FIXME: this map setting should maybe be its own function
    ...
    Spaceship.load(world, this.owner, id, modules); // NOTE: I don't like the look of the line below
  ...
  get centroid() {
    ...
    */
    ...
    let x_pos_total = 0;
    let y_pos_total = 0; // FIXME: I could do it this way since it's more efficient
  ...
  set_projected_torques(centroid) {
    ...
    let motor_strength = 1; // TODO: find a better name, and maybe use a map?

    let module_list = {}; // FIXME: don't know what `F` and `r` are
    ...
    for (let [id, module] of this.modules) {
      switch (module.category) {
        ...
        case "R3":
          ...
          r.y = module.position.y - centroid.y; // TODO: explain what `tau` represents
  ...
  constraint_management(owner, spaceship, a, b, k, d, size) {
    ...
    if (size === 0.5) {
      constraints = Module.h_constraints;
    }

    if (size === 1) {
      constraints = Module.w_constraints;
    }
    ...
    this.constraints.set(a.body + "|" + b.body, Matter.Constraint.create({
      bodyA: this.modules.get(a.body).Matter,
      pointA: {
        x: a.x,
        y: a.y
      },
      bodyB: this.modules.get(b.body).Matter,
      pointB: {
        x: b.x,
        y: b.y
      },
    ...
    this.constraints.get(a.body + "|" + b.body).meta = {
      owner,
      spaceship
    };
    ...
    this.constraints.get(b.body + "|" + a.body).meta = {
      owner,
      spaceship
    };
    return {
  ...
  add_by_type(world, owner, spaceship, id, category, {
    x,
    y,
    d
  }, {
    ...
    switch (category) {
      case "Q1":
        // capsules
        new Q1(world, owner, spaceship, id, {
          x,
          y,
          d
        }, {
      ...
      case "T1":
        // structs
        new T1(world, owner, spaceship, id, {
          x,
          y,
          d
        }, {
      ...
      case "R3":
        // thrusters
        new R3(world, owner, spaceship, id, {
          x,
          y,
          d
        }, {
  ...
  add_modules(world, owner, spaceship, grid, modules, position) {
    ...
    for (let i = 0; i < modules.length; i++) {
      ...
      ({
        ...
        // only for capsules
        main = false
      } = props[1]); // with the remaining first 3 items, make an id and string it
      ...
      this.add_by_type(world, owner, spaceship, id, category, {
        x: coords.x,
        y: coords.y,
        d: coords.d
      }, {
    ...
    this.set_projected_torques(this.centroid); // TODO: add constraints right after module was added instead of waiting for all of them to be added to world

    this.add_constraints(world, owner, spaceship, modules, grid.d, size); // TODO: make check to see if there are any errors in `add_by_type` and `add_constraints` and return bool
  }

  add_constraints(world, owner, spaceship, modules, dir, size) {
    let list = []; //create three ordered lists ordered by xy, yz, zx

    for (let i = 0; i < 3; i++) {
      let j = (i + 1) % 3;
      let k = (i + 2) % 3; // TODO: explain this mess
        ...
        if (a[i] === b[i]) {
          if (a[j] === b[j]) {
            if (Math.abs(a[k] - b[k]) === 1) {
              ...
              this.add_constraint(world, owner, spaceship, a, b, k, dir, size); // add a neighbor

              this.modules.get(a.toString()).neighbors.push(b.toString());
              this.modules.get(b.toString()).neighbors.push(a.toString()); // FIXME: idk what's up with below comment
  ...
  add_constraint(world, owner, spaceship, a, b, k, dir, size) {
    ...
    this.constraints.get(body_pair.a + "|" + body_pair.b), this.constraints.get(body_pair.b + "|" + body_pair.a)]);
  ...
  static load(world, owner, spaceship, modules) {
    ...
    universe.users.get(owner).spaceships.get(spaceship).add_modules(world, owner, spaceship, grid, modules, universe.users.get(owner).spaceships.get(spaceship).position);
  }

  static reset(world, owner, spaceship) {
    // clear bodies
    Matter.Composite.clear(universe.users.get(owner).spaceships.get(spaceship).composite, false, true); // clear virtual bodies
  ...
  static remove_module(world, owner, spaceship, id) {
    ...
    let MatterModule = universe.users.get(owner).spaceships.get(spaceship).modules.get(id).Matter; // remove body

    Matter.Composite.remove(universe.users.get(owner).spaceships.get(spaceship).composite, MatterModule, true);
    universe.users.get(owner).spaceships.get(spaceship).add_by_type( // world, owner, spaceship, id, category, {x, y, d}, {level, interval, size, main}, angle
    world, "", "", "", module.category, {
      ...
      size: module.size
    }, module.angle); // delete virtual body
  ...
  static erase_module(world, owner, spaceship, id) {
    let module = universe.users.get(owner).spaceships.get(spaceship).modules.get(id);
    let matter_module = universe.users.get(owner).spaceships.get(spaceship).modules.get(id).Matter; // remove body

    Matter.Composite.remove(universe.users.get(owner).spaceships.get(spaceship).composite, matter_module, true); // delete virtual body
    // TODO: I don't like this approach, but it's what I could come up with at the moment, figure something out later
  ...
  static remove_constraint(owner, spaceship, id) {
    ...
    Matter.Composite.remove(universe.users.get(owner).spaceships.get(spaceship).composite, universe.users.get(owner).spaceships.get(spaceship).constraints.get(id), true); // TODO: remove from virtual space
