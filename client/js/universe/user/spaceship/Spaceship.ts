...
class Spaceship {
	...
	constraint_management(
		...
		/// order of numeration and comparison
		let o = a[k] - b[k];
	...
	add_constraints(
		...
		dir : DOrientation,
		size : ModuleSize
	) : { xy : TriCoord[], yz : TriCoord[], zx : TriCoord[] } {
		let list : TriCoord[][] = [];
		...
		for (let i = 0; i < 3; i += 1) {
			 ...
			 ** start sorting arrays and add to `list`,
			 ** arrays to be sorted are copies of `mods` + `spaceship.cap_coords` which is `[0,0,0]`
			 **/
			list[i] = [...modules].sort((a, b) => {
				if (a[i] === b[i]) {
					if (a[j] === b[j]) {
						if (Math.abs(a[k] - b[k]) === 1) {
							/** if two consecutive modular number coordinates are followed by a last coordinate
							 ** in both arrays which has a difference of one
	 ...
	 **/
	static tri_to_sqr_coords(origin_d, [ x, y, z ] : TriCoord) {
